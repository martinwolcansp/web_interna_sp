/* ============================================================
   fichas/versiones/nuevas-tecnologias-v1.6.js
   Ficha "Nuevas Tecnologías" — Migración a comodato — V1.6 (08/07/2026)

   Suma sobre v1.5:
   - Decisión de Gerencia: no se van a definir descuentos ni reglas de
     bonificación para el Técnico Comercial, ni un análisis de
     competencia, para este piloto. Se cierran esos tres puntos (dejan
     de ser pendientes) en vez de quedar abiertos:
       · Checklist "Condiciones comerciales": el comentario ya no
         menciona descuentos/bonificación/competencia como faltantes;
         queda solo lo que sigue realmente abierto (qué pasa con el
         abono después de la cuota 6 y validar el recupero con
         números reales).
       · Pestaña Precios → alerta actualizada con el mismo criterio.
       · Pestaña Competencia → nota actualizada: no se releva por
         decisión de Gerencia (antes decía "no se relevó todavía").
       · Pestaña Áreas → Gerencia: se saca de su lista de pendientes
         "definir márgenes de descuento del Técnico Comercial"; el
         comentario de la fila refleja la decisión.
...
window.FICHA_VERSIONS['nuevas-tecnologias']['v1.6'] = {

  /* ── METADATOS DE VERSIÓN (para el selector) ──────────── */
  versionId:   'V1.6',
  versionDesc: 'Descuentos, bonificación y competencia: cerrados por Gerencia',
  ...
  precios: {
    ...
    cancelacion: [
      ...
      { momento: 'Restitución del equipo, dentro de los 10 días de la baja',           penalidad: 'Obligatoria. Si no se restituye (o se restituye dañado/incompleto), el cliente debe abonar su valor — según factura de compra del equipo o, en caso de disputa, tasación de un tercero independiente' },
  ...
  faq: [
    ...
    { q: '¿Qué pasa si el cliente quiere dar de baja el servicio?',
      a: 'Puede dar de baja cuando quiera, con 30 días de preaviso y sin penalidad. Sí tiene que devolver el equipo dentro de los 10 días de la baja; si no lo devuelve, se le cobra su valor (según factura de compra o, en caso de disputa, tasación de un tercero independiente). Como el comodato está atado al monitoreo (conexidad contractual), si se da de baja el monitoreo el comodato se extingue con él.' },
  ...
  areas: [
    ...
    {
      area: 'Gerencia', responsable: '—', comentarios: 'Precio y composición del abono ya definidos ($66.000/mes, ABO-16, instalación sin cargo). Decidió no definir descuentos de bonificación del Técnico Comercial ni análisis de competencia (quedan cerrados); falta definir metas de indicadores y qué ocurre con el abono después de la cuota de recupero', estado: 'rev', fecha: '08/07/2026',
      detalle: {
        titulo: 'Gerencia',
        bloques: [
          { tipo: 'texto', texto: 'Define las condiciones comerciales y los objetivos de negocio del piloto. Decidió no definir descuentos ni reglas de bonificación para el Técnico Comercial, ni un análisis de competencia, para este piloto.' },
          { tipo: 'lista', items: [
    ...
    {
      area: 'Legal', responsable: '—', comentarios: 'Contrato de comodato con el cliente revisado: sin penalidad por baja anticipada (restitución/valor del equipo y conexidad contractual con el monitoreo) — falta cargar el link externo al documento (ver pestaña Precios → Documentos) y completar el correo electrónico para el Derecho de Revocación', estado: 'rev', fecha: '08/07/2026',
      detalle: {
        titulo: 'Legal',
        bloques: [
          { tipo: 'texto', texto: 'El contrato de comodato con el cliente está revisado: duración de 12 meses con renovación mensual automática, sin penalidad por baja anticipada (con obligación de restituir el equipo o abonar su valor), conexidad contractual entre comodato y monitoreo, derecho de revocación (Ley 24.240), ajuste del abono por IPC con índice sustituto, jurisdicción por defecto (art. 36, Ley 24.240) y tratamiento de datos personales (Ley 25.326). El link externo al documento firmado está pendiente de cargar — ver pestaña Precios → Documentos.' },
          { tipo: 'lista', items: [
