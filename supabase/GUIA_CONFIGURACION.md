...
- **Schema + datos + editor de fichas de producto**: `fichas` + `ficha_versiones` (`migracion_4_fichas_producto.sql`, ya en `schema.sql`), contenido real de las 5 fichas existentes migrado con `seed_fichas_producto.sql`, y `pages/fichas-editor.html` + `js/fichas-editor.js` — alta/edición de fichas y versiones, identificación por formulario, las 7 pestañas de contenido por JSON validado, buscador que cruza todas las fichas/versiones/pestañas y lleva directo al campo. Acceso desde el botón "Editor de fichas" en el header (quien tenga permiso de editar mapa-servicios o sector-comunicaciones, superadmin incluido) o directo por URL. **Todavía sin probar contra la base real; las páginas del sitio (Mapa de Servicios, Sector Comunicaciones) siguen leyendo de los archivos estáticos `js/fichas/versiones/*.js`, no de Supabase** — pasar `ficha-renderer.js` a leer de la base es un paso aparte y posterior, a validar una vez que el editor esté probado.
...
6. Migrar el contenido estático de las páginas de cada mosaico a la base de datos — **schema, datos y editor ya construidos** (`migracion_4_fichas_producto.sql` + `seed_fichas_producto.sql` + `pages/fichas-editor.html`, 12 de agosto de 2026), sin probar todavía contra la base real. Falta, en un paso aparte y posterior, pasar `js/fichas/ficha-renderer.js` y las páginas de Mapa de Servicios / Sector Comunicaciones a leer de Supabase en vez de los archivos estáticos `js/fichas/versiones/*.js` — recién ahí el contenido editado en el editor se reflejaría en el sitio.
